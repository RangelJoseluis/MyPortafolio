...
export default function SkillsCategory({ title, skills, delay = 0, isVisible = false }: SkillsCategoryProps) {
    return (
        <div
            className={`bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-cyan-500/30 transition-all duration-700 hover:-translate-y-1 hover:shadow-xl hover:shadow-cyan-500/10 group h-full transform-gpu ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
