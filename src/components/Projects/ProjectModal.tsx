...
export default function ProjectModal({ project, isOpen, onClose }: ProjectModalProps) {
    ...
    return (
        ...
        >
            ...
            {/* Modal Content - Más pequeño */}
            <div
                className="relative bg-[#0a0e27] border border-white/10 rounded-2xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl shadow-cyan-500/10"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header - Padding reducido */}
                <div className="p-4 border-b border-white/10 flex justify-between items-center bg-[#0f1635]">
                    <div>
                        <h3 className="text-xl font-bold text-white mb-1">{project.title}</h3>
                        <div className="flex gap-2">
                            {project.technologies.map((tech, idx) => (
                                <span key={idx} className="text-[10px] px-2 py-0.5 rounded bg-white/5 text-gray-300 border border-white/10">
                                    {tech}
                                </span>
                            ))}
                        </div>
                ...
                {/* Gallery Area - Altura reducida */}
                <div className="flex-1 relative bg-black/20 flex items-center justify-center min-h-[200px] md:min-h-[300px] overflow-hidden group">
                    {/* Main Image */}
                    <div className="relative w-full h-full flex items-center justify-center p-4">
                        <img
                            src={currentImage.url}
                            alt={currentImage.caption}
                            className="max-h-[40vh] md:max-h-[50vh] w-auto object-contain rounded-lg shadow-lg"
                        />
                    </div>
                ...
                {/* Caption & Description Area - Padding reducido */}
                <div className="p-4 bg-[#0f1635] border-t border-white/10">
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-cyan-400 uppercase tracking-wider mb-0.5">
                        ...
                        <div className="mt-1 pt-3 border-t border-white/5">
                            <p className="text-gray-400 text-xs leading-relaxed line-clamp-3 hover:line-clamp-none transition-all cursor-default">
