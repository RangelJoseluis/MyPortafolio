...
export default function ViewAllExperiencesModal({ isOpen, onClose }: ViewAllExperiencesModalProps) {
    ...
    return (
        ...
        >
            ...
            {/* Modal ajustado */}
            <div
                className="relative bg-[#0a0e27]/95 backdrop-blur-2xl border border-white/20 rounded-xl shadow-2xl shadow-cyan-500/20 max-w-4xl w-full max-h-[85vh] flex flex-col transform-gpu"
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex-1">
                                            ...
                                            <p className="text-cyan-200 font-medium text-xs md:text-sm">{exp.company}</p>
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                ...
                                                <span className="px-2 py-0.5 rounded-full bg-white/5 text-gray-300 text-[10px] border border-white/10">
                                                    📍 {exp.location}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="px-4 pb-4 pt-2 border-t border-white/10">
                                            ...
                                            <div className="mb-4">
                                                <h4 className="text-xs font-bold text-white mb-2 flex items-center gap-2">
                                                    <span className="w-1 h-3 bg-gradient-to-b from-cyan-400 to-blue-600 rounded-full"></span>
                                                    Logros y Responsabilidades
                                                </h4>
                                                <ul className="space-y-1.5">
                                                    {exp.achievements.map((achievement, achIdx) => (
                                                        <li key={achIdx} className="flex items-start gap-2 text-gray-300 text-xs">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-1.5">
                                                        {exp.tech.map((tech, techIdx) => (
                                                            <span
                                                                key={techIdx}
                                                                className="px-2 py-0.5 text-[10px] font-semibold text-white bg-gradient-to-r from-cyan-500/10 to-blue-600/10 rounded border border-cyan-500/20"
                                                            >
                                                                {tech}
                                                            </span>
                                                        ))}
